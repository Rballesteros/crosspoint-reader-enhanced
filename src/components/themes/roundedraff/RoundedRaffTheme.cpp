...
void RoundedRaffTheme::drawHeader(const GfxRenderer& renderer, Rect rect, const char* title,
  ...
  if (showBatteryPercentage) {
    batteryGroupLeftX -= maxBatteryTextWidth + batteryPercentSpacing;
  }
  ...
  if (showBatteryPercentage || showBluetooth) {
    ...
    const int clearLeftX = batteryIconX - (showBatteryPercentage ? maxBatteryTextWidth + batteryPercentSpacing : 0) -
